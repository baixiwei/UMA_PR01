...
R_VM_next_calc = Rule( # In VM state NEXT_CALC, do next calculation
    ...
    acts    = [# set algorithm state to END_CALC
               ...
               # {'action':       'create_chunk',
                ...
                # 'features':     {
                    # 'operator':     '*',
                    # 'operand1':     Variable('op1dig'),
                    # 'operand2':     Variable('op2dig')}},
               # {'action':       'set_feature',
                # 'chunk':        Variable('vproblem'),
                # 'slot':         'curr_calc',
                # 'value':        Variable('new_calc')},
               # {'action':       'push_goal',
                # 'goal':         Goal('solve_problem', {
                    # 'problem':      Variable('new_calc')})}])
...
R_VM_end_calc = Rule( # In VM state END_CALC, end current calculation
    ...
    conds   = [# algorithm is VM
               ...
               {'target':       Variable('vproblem'),
                'pattern':      pat_vprob},
               # Originally curr_ans was extracted from curr_calc
               # Now, it is stored separately in vprob, making the following unnecessary
               # {'target':       Variable('curr_calc'),
                # 'pattern':      Problem(features={'answer': Variable('curr_ans')})},
...
R_align_dec_omit = Rule( # Supposed to align decimal operands at decimal point, but omit this specification, so revert to default alignment (i.e., right)
    name    = 'align_dec_omit',
    goal    = Goal('align_operands', {
                'vproblem': Variable('vproblem'),
                'align':    'decimal'}),
...
R_get_LCM = Rule( # Get LCM and store it in 'LCM' slot of current goal
    ...
    conds   = [],
    acts    = [{'action':   'pop_goal'},
               # Like FARRA, we cheat by using a primitive action to determine LCM
               {'action':   'get_LCM',
                'number1':  Variable('number1'),
                'number2':  Variable('number2'),
                'chunk':    Variable('curr_goal'),
...
R_get_GCD = Rule( # Get GCD and store it in 'GCD' slot of current goal
    ...
    conds   = [],
    acts    = [{'action':   'pop_goal'},
               # Like FARRA, we cheat by using a primitive action to determine GCD
               {'action':   'get_GCD',
                'number1':  Variable('number1'),
                'number2':  Variable('number2'),
                'chunk':    Variable('curr_goal'),
