...
def profile_training_set(f='new_go_math'):

    P = ProblemSet.load(f)
    for v in ['notation', 'operation', 'operands', 'denoms']:
        P[v] = P[v].astype('category')

    notations = [n for n in ['whole', 'fraction', 'decimal'] if n in list(P['notation'])]
    if len(notations)>0:
        P['notation'] = P['notation'].cat.reorder_categories(notations)

    operations = [o for o in ['add', 'sub', 'mul', 'div'] if o in list(P['operation'])]
    if len(operations)>0:
        P['operation'] = P['operation'].cat.reorder_categories(operations)

    operands = [o for o in ['SD', 'MD', 'FR', 'MI', 'WF', 'DD', 'WD'] if o in list(P['operands'])]
    if len(operands)>0:
        P['operands'] = P['operands'].cat.reorder_categories(operands)

    denoms = [d for d in ['', 'ED', 'UD', 'EDD', 'UDD'] if d in list(P['denoms'])]
    if len(denoms)>0:
        P['denoms'] = P['denoms'].cat.reorder_categories(denoms)

    return(pd.crosstab(index=[P['notation'], P['operation']], columns=[P['operands'], P['denoms']]))

def do_sim_202206_final(start_subjid=0, end_subjid=None):
    ps      = {
       'g':     [.05, .06],
       'd':     [.3, .7],
       'c':     [5],
       ...
       # 'rt_sd': [1],
       # 'ice':   [0, 25, 50, 75, 100]}
    rules = [rule for rule in all_rules if rule.name!=R_align_dec_omit.name]
    S = Simulation("202206_final", params_sets=ps, rules=rules, N=1, curriculum="full")
    S.run(start_subjid, end_subjid)

do_sim_202206_final()
Simulation.mergeResults("202206_final")